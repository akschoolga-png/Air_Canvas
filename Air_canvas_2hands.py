...
options = vision.HandLandmarkerOptions(
    ...
    num_hands=2,
    min_hand_detection_confidence=0.5,  # Changed parameter name
    min_hand_presence_confidence=0.5,    # Added this parameter
...
# Deques to store drawing points for each color
# We use a list of lists of deques to handle "breaks" in drawing
...
while True:
    ...
    # UI: Draw the color selection buttons
    ...
    if result.hand_landmarks:
        for hand_landmarks in result.hand_landmarks:
            ...
            # Check if finger is in the button area
            if cy <= 65:
                ...
                # If we just started drawing, add a new deque
        ...
        # If hand is not detected, add an empty deque to create a "break" in the line
        for i in range(4):
            if points[i] and len(points[i][-1]) > 0:
                points[i].append(deque(maxlen=512))

    # Draw the lines on both windows
